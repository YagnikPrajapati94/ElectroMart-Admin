...
const AddBrandModel = ({ fetchBrands, editData }) => {
    ...
    const handleFormSubmit = async (data) => {
        ...
        try {
            if (editData) {
                const res = await axios.put(`${baseURL}/api/updateBrand/${editData._id}`, data, {
                    headers: {
                    ...
                    <div className="modal-body rounded-3 ">
                        <form onSubmit={handleSubmit(handleFormSubmit)} className='form-control bg-transparent text-light border-0 d-grid gap-2'>
                            ...
                            <div className="mb-3">
                                ...
                                <input
                                    type="file"
                                    className={`form-control ${errors.brandLogo ? 'is-invalid' : ''}`}
                                    accept="image/*"
                            ...
                            {/* Submit Button */}
                            <button type="submit" className={`btn shadow-none ${editData ? 'btn-warning' : 'btn-primary'} ${loading ? 'disabled' : ''} form-control`}>
                                {loading ? (
