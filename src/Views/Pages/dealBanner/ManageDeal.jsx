...
const ManageDeal = () => {
  ...
  const fetchCategories = async () => {
    setLoading(true);
    try {
      const res = await promotionsService.getDeals();
      setCategories(res.data.deals);
    } catch (error) {
      console.log(error);
    } finally {
      setLoading(false);
  ...
  const handleDelete = async (id) => {
    try {
      ...
      if (!swalResult.isConfirmed) {
        ...
        speak("Deal deletion cancelled");
        return;
      }

      // ✅ API call
      const response = await promotionsService.deleteDeal(id);

      if (response.data.success) {
        Swal.fire("Deleted!", "Your file has been deleted.", "success");
        speak("Deal deleted successfully");
  ...
  const handleEdit = (id) => {
    navigation(`/admin/deal/add/${id}`);
  };

  const handleToggle = async (id) => {
    setLoading(true);
    try {
      const response = await promotionsService.updateDealStatus(id);
    ...
    <AdminLayout>
      <div className="container-fluid p-4">
        <div className="row mb-4 px-2 align-items-center">
          <BreadCrumb parent={"Deals"} child={"List"} />
          <div className="col-12 text-center p-0">
            <h3 className="fw-semibold TitleText m-0">Deals Directory</h3>
            <small className="SubtitleText">
              View, search, and manage all Added Deals here.
            </small>
          </div>
        </div>
                  ...
                  {loading == true ? (
                    ...
                    <tr>
                      <td colSpan="5" className="text-center py-4">
                        <i className="bi bi-info-circle me-2"></i> No Deals
                        found.
                      </td>
                    </tr>
                  )}
