...
const AddDeal = () => {
  ...
  const handleCategorySubmit = async (data) => {
    ...
    try {
      if (editId) {
        const res = await promotionsService.updateDeal(editId, data);
        if (res.data.success) {
          speak("Deal updated successfully");
          toast.success("Deal updated successfully");
          reset();
          setPreview(null);
          navigation("/admin/deal/manage");
        }
      } else {
        const result = await promotionsService.addDeal(data);
        if (result.data.success) {
          speak("Deal added successfully");
  ...
  const fetchCategoryById = async (editId) => {
    try {
      const result = await promotionsService.getDealById(editId);
      if (result.data.success) {
      ...
      <div className="container-fluid p-4  ">
        <div className="row mb-5 px-2  justify-content-center">
          ...
          <div className="col-lg-12 p-0  rounded-2 ">
            ...
            >
              <h4 className="text-start TitleText fw-semibold">Add Deal</h4>
              <p className="SubtitleText small mb-4">
                Create a new deal by selecting a product and defining its offer
                details such as discount, validity period, and display priority.
                This deal will be shown to customers to highlight special
                product offers and increase sales.
              </p>
